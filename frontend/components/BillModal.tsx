...
export function BillModal({ order, onClose, onPaid, payOnly }: {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: "16px" }}>
      <div style={{ background: "var(--surface)", border: "1px solid #252535", borderRadius: "20px", width: "100%", maxWidth: "460px", maxHeight: "90vh", direction: "rtl", overflow: "hidden", display: "flex", flexDirection: "column" }}>

        {/* Header */}
        <div style={{ padding: "18px 22px", borderBottom: "1px solid #252535", background: "rgba(245,158,11,0.04)", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
            <div>
              <div style={{ color: "var(--text)", fontWeight: "800", fontSize: "17px" }}>🧾 فاتورة طلب #{order.id}</div>
              <div style={{ color: "var(--muted)", fontSize: "12px", marginTop: "3px" }}>🪑 طاولة {order.table_number}</div>
            </div>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          ...
          {splitMode === "none" && (<>
            <div style={{ padding: "14px 22px" }}>
              ...
              {order.notes && (
                <div style={{ marginTop: "10px", background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: "8px", padding: "7px 10px", color: "var(--gold)", fontSize: "12px" }}>
                  📝 {order.notes}
                </div>
              )}
            </div>
            ...
            <div style={{ padding: "16px 22px" }}>
              <div style={{ marginBottom: "16px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <button onClick={() => changeNumPeople(numPeople - 1)} disabled={numPeople <= 2} style={{ width: "40px", height: "40px", borderRadius: "10px", fontSize: "20px", fontWeight: "700", background: "var(--raised)", color: numPeople <= 2 ? "var(--subtle)" : "var(--gold)", border: "1px solid #252535", cursor: numPeople <= 2 ? "not-allowed" : "pointer" }}>−</button>
                  ...
                  <button onClick={() => changeNumPeople(numPeople + 1)} style={{ width: "40px", height: "40px", borderRadius: "10px", fontSize: "20px", fontWeight: "700", background: "var(--raised)", color: "var(--gold)", border: "1px solid #252535", cursor: "pointer" }}>+</button>
                </div>
              </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {equalPaid[i] ? (
                        ...
                        <div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
                          ...
                          <button onClick={() => payEqualGuest(i)} style={{ padding: "6px 14px", background: "rgba(34,197,94,0.15)", color: "var(--green)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "9px", cursor: "pointer", fontSize: "12px", fontWeight: "700" }}>دفع</button>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {splitMode === "items" && (
            <div style={{ padding: "16px 22px" }}>
              {items.length === 0 ? (
                <div style={{ color: "var(--muted)", textAlign: "center", padding: "24px 0" }}>لا توجد تفاصيل أصناف لهذا الطلب</div>
              ) : (<>
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ color: "var(--text2)", fontSize: "12px", marginBottom: "10px" }}>عدد الزبائن</div>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <button onClick={() => changeNumGuests(numGuests - 1)} disabled={numGuests <= 2} style={{ width: "40px", height: "40px", borderRadius: "10px", fontSize: "20px", fontWeight: "700", background: "var(--raised)", color: numGuests <= 2 ? "var(--subtle)" : "var(--gold)", border: "1px solid #252535", cursor: numGuests <= 2 ? "not-allowed" : "pointer" }}>−</button>
                    <input type="number" min={2} value={numGuests} onChange={e => changeNumGuests(parseInt(e.target.value) || 2)}
                      style={{ flex: 1, textAlign: "center", padding: "10px", borderRadius: "10px", background: "rgba(245,158,11,0.08)", color: "var(--gold)", border: "1px solid rgba(245,158,11,0.35)", fontSize: "18px", fontWeight: "800", outline: "none" }} />
                    <button onClick={() => changeNumGuests(numGuests + 1)} style={{ width: "40px", height: "40px", borderRadius: "10px", fontSize: "20px", fontWeight: "700", background: "var(--raised)", color: "var(--gold)", border: "1px solid #252535", cursor: "pointer" }}>+</button>
                  </div>
                </div>
