...
def group(group_id: int) -> ModifierGroup:
    try:
...
def option(option_id: int) -> ModifierOption:
    try:
...
def add_item(
    *, name: str, price, category: str, description: str, parent_id: Optional[int]
) -> MenuItem:
    """Add a dish, or a Variant when ``parent_id`` names one of this Restaurant's dishes."""
    parent = item(parent_id) if parent_id is not None else None
    return MenuItem.objects.create(
        name=name, price=price, category=category, description=description, parent=parent
    )


def edit_item(item_id: int, *, name: str, price, category: str, description: str) -> MenuItem:
...
def create_group(item_id: int, *, name: str, max_selections: int) -> ModifierGroup:
    return ModifierGroup.objects.create(
        menu_item=item(item_id), name=name, max_selections=max_selections
    )


def edit_group(group_id: int, *, name: str, max_selections: int) -> ModifierGroup:
    edited = group(group_id)
...
def delete_group(group_id: int) -> None:
    group(group_id).delete()  # the group's options go with it (cascade)


def create_option(
    group_id: int, *, name: str, price_delta, inventory_item_id: Optional[int], quantity_delta
) -> ModifierOption:
...
def edit_option(option_id: int, *, name: str, price_delta) -> ModifierOption:
    edited = option(option_id)
...
def reorder_groups(item_id: int, order: list) -> None:
    ...
    _apply_order(item(item_id).modifier_groups, order)


def reorder_options(group_id: int, order: list) -> None:
    _apply_order(group(group_id).options, order)
